...
import { Plus, Edit2, Trash2, Settings, Database, Key, Calculator, X, Package, DollarSign, ShoppingCart, FolderTree, Users, Building, Search } from 'lucide-react'
...
export default function ERPConfigurationModuleComplete() {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const [mgRes, vcRes, ptRes, uomRes, msRes, glRes, valRes, mtRes, ccRes, plantRes, slRes, adRes] = await Promise.all([
        ...
        supabase.from('storage_locations').select('*, plant:plants(plant_code, plant_name)').order('sloc_code'),
        // Get account determination with separate queries for lookup data
        supabase.from('account_determination').select('*')
      ])

      // Get lookup data for account determination
      const [companiesRes, valuationRes, accountKeysRes, glAccountsRes] = await Promise.all([
        supabase.from('company_codes').select('id, company_code'),
        supabase.from('valuation_classes').select('id, class_code'),
        supabase.from('account_keys').select('id, account_key_code'),
      ...
      if (valRes.data) setValuationClasses(valRes.data)
      if (mtRes.data) setMovementTypes(mtRes.data)
      if (ccRes.data) setCompanyCodes(ccRes.data)
      if (plantRes.data) setPlants(plantRes.data)
      if (adRes.data && companiesRes.data && valuationRes.data && accountKeysRes.data && glAccountsRes.data) {
        console.log('Account Determination raw data:', adRes.data)
        
        // Create lookup maps
        const companyMap = new Map(companiesRes.data.map(c => [c.id, c.company_code]))
        const valuationMap = new Map(valuationRes.data.map(v => [v.id, v.class_code]))
        const accountKeyMap = new Map(accountKeysRes.data.map(a => [a.id, a.account_key_code]))
        const glAccountMap = new Map(glAccountsRes.data.map(g => [g.id, { code: g.account_code, name: g.account_name }]))
        
        const formattedAD = adRes.data.map(ad => ({
          ...ad,
          company_code: companyMap.get(ad.company_code_id) || 'N/A',
          valuation_class_code: valuationMap.get(ad.valuation_class_id) || 'N/A',
          account_key_code: accountKeyMap.get(ad.account_key_id) || 'N/A',
          gl_account_code: glAccountMap.get(ad.gl_account_id)?.code || 'N/A',
          gl_account_name: glAccountMap.get(ad.gl_account_id)?.name || 'N/A'
        }))
        console.log('Formatted Account Determination:', formattedAD)
        setAccountDetermination(formattedAD)
      } else {
        console.log('No account determination data found:', adRes.error)
      }
    } catch (error) {
            ...
            <>
                        ...
                        <table className="w-full">
                          ...
                          <tbody>
                            {accountDetermination.slice(0, 10).map((item) => (
                              <tr key={item.id} className="border-b hover:bg-gray-50">
                                <td className="py-2 px-3 text-sm font-mono">{item.company_code}</td>
                                <td className="py-2 px-3 text-sm">
                                  <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs">
                                    {item.valuation_class_code}
                                  </span>
                                </td>
                                <td className="py-2 px-3 text-sm">
                                  <span className="px-2 py-1 bg-green-100 text-green-800 rounded text-xs">
                                    {item.account_key_code}
                                  </span>
                                </td>
                                <td className="py-2 px-3 text-sm font-mono">{item.gl_account_code}</td>
                                <td className="py-2 px-3 text-sm">
                                  <span className={`px-2 py-1 rounded text-xs ${
                                    item.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                                  }`}>
              ...
              {activeTab === 'projects' && (
                <div className="bg-white rounded-lg shadow-sm border p-8 text-center">
                  ...
                  <p className="text-gray-500">Project configuration coming soon...</p>
                  <p className="text-sm text-gray-400">WBS types, Activity categories, Task types</p>
                </div>
              )}
              {activeTab === 'hr' && (
                <div className="bg-white rounded-lg shadow-sm border p-8 text-center">

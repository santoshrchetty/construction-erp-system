...
import { generateActivityCode } from '../lib/code-generator';
...
export default function ActivityManager({ projectId }: { projectId: string }) {
  ...
  const fetchActivities = async () => {
    ...
    if (data) {
      const activitiesWithWBS = data.map(activity => ({
        ...activity,
        wbs_node_name: activity.wbs_nodes?.name || 'No WBS',
        vendor_name: activity.vendors?.name || null
      }));
      setActivities(activitiesWithWBS);
    }
    setLoading(false);
  };
                ...
                <div>
                  ...
                  <div className="flex space-x-2">
                    <input
                      ...
                      placeholder="Will auto-generate when WBS selected"
                      required
                    />
                    <button
                      type="button"
                      onClick={async () => {
                        if (formData.wbs_node_id) {
                          ...
                          const code = await generateActivityCode(projectId, formData.wbs_node_id)
                          setFormData({...formData, code})
                          setIsGeneratingCode(false)
                        }
                      }}
                      disabled={!formData.wbs_node_id || isGeneratingCode}
                      className="px-3 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 disabled:opacity-50 text-sm"
                    >
                      {isGeneratingCode ? '...' : '🔄'}
                    </button>
                  </div>
                  <p className="text-xs text-gray-500 mt-1">Select WBS first, then click 🔄 to auto-generate</p>
                </div>
                <div>
                  ...
                  <select
                    ...
                    onChange={async (e) => {
                      const wbsNodeId = e.target.value
                      setFormData({...formData, wbs_node_id: wbsNodeId})
                      
                      // Auto-generate code when WBS is selected
                      if (wbsNodeId && !editingActivity) {
                        setIsGeneratingCode(true)
                        const code = await generateActivityCode(projectId, wbsNodeId)
                        setFormData(prev => ({...prev, wbs_node_id: wbsNodeId, code}))
                        setIsGeneratingCode(false)
                      }
                    }}

...
export default function ERPConfigurationModuleComplete() {
  ...
  const plantsByCompany = useMemo(() => {
    ...
    plants.forEach(plant => {
      const companyCode = plant.company_code
      if (!map.has(companyCode)) map.set(companyCode, [])
      map.get(companyCode)!.push(plant)
    })
    return map
  }, [plants])
      ...
      if (modalEntity === 'companies') {
        ...
        return (
          <>
            ...
            <select name="company_code" className="w-full p-2 border rounded mb-3" required defaultValue={editingItem?.company_code || ''}>
              <option value="">Select Company</option>
              {companyCodes.map(cc => (
                <option key={cc.id} value={cc.company_code}>{cc.company_code} - {cc.company_name}</option>
              ))}
            </select>
                      ...
                      <div className="overflow-x-auto">
                        ...
                        {accountDetermination.length === 0 ? (
                          ...
                          accountDetermination.slice(0, 10).map((item) => {
                            const company = companyCodes.find(c => c.company_code === item.company_code)
                ...
                <div className="bg-white rounded-lg shadow-sm border p-4">
                  ...
                  <div className="space-y-2 max-h-96 overflow-y-auto">
                    ...
                    {companyCodes.map((company) => {
                      const companyPlants = plantsByCompany.get(company.company_code) || []

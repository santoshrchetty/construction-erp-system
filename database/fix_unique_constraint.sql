-- First, add the account assignment category column
...
-- Add new unique constraint that includes account assignment category
ALTER TABLE movement_type_account_keys ADD CONSTRAINT movement_type_account_keys_unique 
UNIQUE(movement_type_id, account_key_id, debit_credit_indicator, COALESCE(account_assignment_category, ''));

-- Update existing records to have NULL account assignment (normal stock)
UPDATE movement_type_account_keys SET account_assignment_category = NULL WHERE account_assignment_category IS NULL;

-- Now add the project variants

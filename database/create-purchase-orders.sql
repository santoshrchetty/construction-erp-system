-- PURCHASE ORDER COMPREHENSIVE SCHEMA
...
CREATE INDEX IF NOT EXISTS idx_po_company_code ON purchase_orders(company_code);
CREATE INDEX IF NOT EXISTS idx_po_supplier_id ON purchase_orders(supplier_id);
CREATE INDEX IF NOT EXISTS idx_po_status ON purchase_orders(status);
CREATE INDEX IF NOT EXISTS idx_po_items_po_id ON purchase_order_items(po_id);
CREATE INDEX IF NOT EXISTS idx_po_items_material ON purchase_order_items(material_code);

-- ========================================
-- 6. PO NUMBER GENERATION FUNCTION
-- ========================================

CREATE OR REPLACE FUNCTION generate_po_number(p_company_code VARCHAR(10))
...
BEGIN
  ...
  -- Format: PO-C001-001000
  v_po_number := 'PO-' || p_company_code || '-' || LPAD(v_sequence_number::TEXT, 6, '0');
...
-- ========================================
-- 7. PO TOTAL CALCULATION FUNCTION
-- ========================================
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(line_amount + total_tax_amount), 0)
...
-- ========================================
-- 8. PO STATUS UPDATE TRIGGER
-- ========================================

CREATE OR REPLACE FUNCTION update_po_item_status()
RETURNS TRIGGER AS $$
BEGIN
  -- Update pending quantity
  NEW.pending_quantity := NEW.ordered_quantity - NEW.received_quantity;
  
  -- Update item status
  IF NEW.received_quantity >= NEW.ordered_quantity THEN
    NEW.item_status := 'CLOSED';
  ELSIF NEW.received_quantity > 0 THEN
    NEW.item_status := 'PARTIAL';
  ELSE
    NEW.item_status := 'OPEN';
  END IF;
...
CREATE TRIGGER po_item_status_trigger
  BEFORE INSERT OR UPDATE ON purchase_order_items
  FOR EACH ROW EXECUTE FUNCTION update_po_item_status();
...
('PO_APPROVE', 'Purchase Order Approve', 'MM', true),
('PO_VIEW', 'Purchase Order View', 'MM', true),
('PO_MODIFY', 'Purchase Order Modify', 'MM', true)
...
-- Insert sample PO statuses
INSERT INTO purchase_orders (
  po_number, company_code, supplier_id, po_date, status, 
  subtotal_amount, tax_amount, total_amount, created_by
) 
SELECT 
  'PO-C001-001000', 'C001', v.id, CURRENT_DATE, 'DRAFT',
...
SELECT 'PURCHASE ORDER SCHEMA CREATED SUCCESSFULLY' as status;
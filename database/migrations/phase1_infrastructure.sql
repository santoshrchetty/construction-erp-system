...
-- Step 3: Update document_number_ranges table
...
SELECT 
    ...
    current_number,
    to_number,
    to_number::BIGINT - current_number::BIGINT as remaining,
    ROUND((current_number::NUMERIC / to_number::NUMERIC) * 100, 2) as utilization_pct,
    CASE 
        WHEN current_number::BIGINT >= to_number::BIGINT THEN '🔴 EXHAUSTED'
        WHEN (current_number::NUMERIC / to_number::NUMERIC) * 100 >= critical_threshold THEN '🟠 CRITICAL'
        WHEN (current_number::NUMERIC / to_number::NUMERIC) * 100 >= warning_threshold THEN '🟡 WARNING'
        ELSE '🟢 OK'
...
-- Step 7: Create improved RPC function
...
-- Step 8: Create fiscal year helper function
...
-- Completion message
SELECT 'Phase 1: Infrastructure Created Successfully!' as status;

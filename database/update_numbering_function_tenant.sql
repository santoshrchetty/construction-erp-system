-- Update function to accept tenant_id as parameter instead of hardcoding it
CREATE OR REPLACE FUNCTION public.get_next_document_number(
  p_company_code text,
  p_document_type text,
  p_number_range_group text,
  p_fiscal_year text,
  p_tenant_id uuid DEFAULT '9bd339ec-9877-4d9f-b3dc-3e60048c1b15'::uuid
) RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  IF v_current_number IS NULL THEN
    INSERT INTO public.document_number_ranges (
      ...
      p_tenant_id,
      1,
      'Auto-created number range for ' || p_document_type
    );
    v_next_number := 1;

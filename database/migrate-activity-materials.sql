-- =====================================================
-- ACTIVITY MATERIALS - Migration to Aligned Schema
-- =====================================================

-- Drop existing table and recreate with aligned schema
DROP TABLE IF EXISTS activity_materials CASCADE;

-- Create activity_materials table (aligned with reservations and stock)
...
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.status = 'reserved' AND (OLD IS NULL OR OLD.status != 'reserved') THEN
    ...
    IF OLD IS NOT NULL AND OLD.status = 'reserved' AND NEW.status != 'reserved' THEN
        UPDATE stock_balances sb
...
SELECT 'Activity Materials table created with aligned schema' as status;

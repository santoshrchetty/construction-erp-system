-- =====================================================
-- CONSTRUCTION MANAGEMENT SAAS - UNIFIED SCHEMA v2.0
-- Single Source of Truth - Updated with Latest Fixes
-- =====================================================
-- Version: 2.0
-- Last Updated: 2024
...
-- Company Code to Controlling Area Assignment (uses existing controlling_areas)
...
-- Activities (Fixed FK constraint)
...
CREATE INDEX idx_company_controlling_company ON company_controlling_areas(company_code_id);
...
CREATE INDEX idx_journal_entries_profit_center ON journal_entries(profit_center);
CREATE INDEX idx_movement_type_mappings_company ON movement_type_account_mappings(company_code);
CREATE INDEX idx_document_ranges_company_type ON document_number_ranges(company_code, document_type);
...
CREATE INDEX idx_cost_centers_profit_center ON cost_centers(profit_center_id);
CREATE INDEX idx_cost_centers_controlling_area ON cost_centers(controlling_area_id);
...
-- Document number ranges validation
...
COMMENT ON SCHEMA public IS 'Construction Management SaaS - Unified Database Schema v2.0';
COMMENT ON TABLE company_codes IS 'Legal entities/companies in the system';
COMMENT ON TABLE projects IS 'Construction projects with SAP organizational assignments and profit centers';
COMMENT ON TABLE wbs_nodes IS 'Work Breakdown Structure hierarchy';
COMMENT ON TABLE activities IS 'Schedulable work units with dependencies and proper vendor references';
...
COMMENT ON TABLE company_controlling_areas IS 'Company code to controlling area assignments';

-- Schema creation completed
SELECT 'Unified Construction Management Schema v2.0 Created Successfully!' as status;